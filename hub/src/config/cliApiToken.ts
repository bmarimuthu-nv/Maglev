 ...
 *
 * Handles automatic generation and persistence of MAGLEV_API_TOKEN.
...
function normalizeCliApiToken(rawToken: string, source: CliApiTokenSource): { token: string; didStrip: boolean } {
    ...
    if (!parsed) {
        if (rawToken.includes(':')) {
            console.warn(`[WARN] MAGLEV_API_TOKEN from ${source} contains ":" but is not a valid token. Server expects a base token without namespace.`)
        }
    ...
    console.warn(
        `[WARN] MAGLEV_API_TOKEN from ${source} includes namespace suffix "${parsed.namespace}". ` +
 ...
 *
 * Priority:
 * 1. MAGLEV_API_TOKEN environment variable (highest - backward compatible)
...
export async function getOrCreateCliApiToken(dataDir: string): Promise<CliApiTokenResult> {
    ...
    if (envToken) {
        ...
        if (isWeakToken(normalized.token)) {
            console.warn('[WARN] MAGLEV_API_TOKEN appears to be weak. Consider using a stronger secret.')
        }
