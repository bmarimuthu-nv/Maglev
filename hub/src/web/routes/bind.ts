...
export function createBindRoutes(jwtSecret: Uint8Array, store: Store, options?: { remoteMode?: boolean }): Hono<WebAppEnv> {
    const app = new Hono<WebAppEnv>()

    app.post('/bind', async (c) => {
        if (options?.remoteMode) {
            return c.json({ error: 'Telegram binding is disabled in remote mode' }, 403)
        }
