...
export interface MaglevBotConfig {
...
/**
 * Maglev Telegram Bot - Notification-only mode
 */
export class MaglevBot implements NotificationChannel {
    ...
    constructor(config: MaglevBotConfig) {
    ...
    async start(): Promise<void> {
        ...
        console.log('[MaglevBot] Starting Telegram bot...')
        this.isRunning = true

        // Start polling
        this.bot.start({
            onStart: (botInfo) => {
                console.log(`[MaglevBot] Bot @${botInfo.username} started`)
            }
        })
    }
    ...
    async stop(): Promise<void> {
        ...
        console.log('[MaglevBot] Stopping Telegram bot...')
    ...
    private setupMiddleware(): void {
        ...
        this.bot.catch((err) => {
            console.error('[MaglevBot] Error:', err.message)
        })
    }
    ...
    private setupCommands(): void {
        ...
        this.bot.command('app', async (ctx) => {
            ...
            await ctx.reply('Open Maglev Mini App:', { reply_markup: keyboard })
        })
    ...
    async sendReady(session: Session): Promise<void> {
        ...
        for (const chatId of chatIds) {
            try {
                ...
                console.error(`[MaglevBot] Failed to send ready notification to chat ${chatId}:`, error)
            }
        }
    }
    ...
    async sendPermissionRequest(session: Session): Promise<void> {
        ...
        for (const chatId of chatIds) {
            try {
                ...
                console.error(`[MaglevBot] Failed to send notification to chat ${chatId}:`, error)
            }
        }
    }
}
