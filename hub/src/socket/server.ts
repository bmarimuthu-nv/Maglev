...
import { TerminalStateCache } from '../sync/terminalStateCache'
...
export function createSocketServer(deps: SocketServerDeps): {
    ...
    const idleTimeoutMs = resolveEnvNumber('MAGLEV_TERMINAL_IDLE_TIMEOUT_MS', DEFAULT_IDLE_TIMEOUT_MS)
    const maxTerminals = resolveEnvNumber('MAGLEV_TERMINAL_MAX_TERMINALS', DEFAULT_MAX_TERMINALS)
    const maxTerminalsPerSocket = maxTerminals
    const maxTerminalsPerSession = maxTerminals
    const cliNs = io.of('/cli')
    const terminalNs = io.of('/terminal')
    const terminalStateCache = new TerminalStateCache()
    ...
    terminalNs.on('connection', (socket) => registerTerminalHandlers(socket, {
        ...
        onSessionTerminalInput: deps.onSessionTerminalInput
    }))

    return { io, engine, rpcRegistry, terminalStateCache }
}
