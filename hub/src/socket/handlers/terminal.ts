...
export function registerTerminalHandlers(socket: SocketWithData, deps: TerminalHandlersDeps): void {
    const { io, getSession, terminalRegistry, maxTerminalsPerSocket, maxTerminalsPerSession, onSessionTerminalInput } = deps
    ...
    socket.on('terminal:create', (data: unknown) => {
        ...
        const { sessionId, terminalId, cols, rows, force, createIfMissing } = parsed.data
        ...
        if (existing) {
            ...
            const previousSocketId = existing.socketId && existing.socketId !== socket.id ? existing.socketId : null
            const reattached = terminalRegistry.reattach(terminalId, sessionId, socket.id, cliSocketId)
            if (!reattached) {
    ...
    socket.on('terminal:write', (data: unknown) => {
        ...
        terminalRegistry.markActivity(terminalId)
        onSessionTerminalInput?.({ sessionId: entry.sessionId, terminalId, actor: 'human' })
    })
