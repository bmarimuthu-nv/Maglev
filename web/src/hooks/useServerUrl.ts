...
import { getBrokerBasepath, normalizeUrlToBrokerBase } from '@/utils/url'

const HUB_URL_KEY = 'maglev_hub_url'

export type ServerUrlResult =
    | { ok: true; value: string }
    | { ok: false; error: string }

function normalizePreservingPath(url: URL): string {
    return normalizeUrlToBrokerBase(url, '')
}

function normalizeToCurrentBrokerBase(url: URL): string {
    if (typeof window === 'undefined') {
        return normalizePreservingPath(url)
    }
    return normalizeUrlToBrokerBase(url, getBrokerBasepath(window.location.pathname))
}
...
export function normalizeServerUrl(input: string): ServerUrlResult {
    ...
    return { ok: true, value: normalizePreservingPath(parsed) }
}
...
export function useServerUrl(): {
    ...
    const fallbackBaseUrl = typeof window !== 'undefined'
        ? normalizeToCurrentBrokerBase(new URL(window.location.href))
        : ''
    const baseUrl = useMemo(() => serverUrl ?? fallbackBaseUrl, [serverUrl, fallbackBaseUrl])
