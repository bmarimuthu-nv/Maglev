...
export default function FilePage() {
    ...
    const diffQuery = useQuery({
        queryKey: queryKeys.gitFileDiffScoped(scopeKey, sessionId, filePath, staged),
        queryFn: async () => {
    ...
    const fileHash = fileContentResult?.success ? (fileContentResult.hash ?? null) : null
    ...
    const isDirty = isEditing && draftContent !== decodedContent
    const canEdit = Boolean(fileContentResult?.success && !binaryFile)
    ...
    const handleSave = async () => {
        ...
        try {
            const result = await api.writeSessionFile(sessionId, filePath, encodeBase64(draftContent), fileHash)
            if (!result.success) {
                throw new Error(result.error ?? 'Failed to save file')
            }

            setIsEditing(false)
            void queryClient.invalidateQueries({ queryKey: queryKeys.sessionFile(scopeKey, sessionId, filePath) })
            void queryClient.invalidateQueries({ queryKey: queryKeys.gitFileDiffScoped(scopeKey, sessionId, filePath, staged) })
                        ...
                        <div className="ml-auto flex items-center gap-2">
                            {isEditing ? (
                                ...
                                <button
                                    ...
                                    className="rounded px-3 py-1 text-xs font-semibold bg-[var(--app-subtle-bg)] text-[var(--app-fg)]"
                                >
                                    Edit
                                </button>
                            ) : null}
