...
const RELEASE_URL = process.env.MAGLEV_RELEASE_URL || '';
...
function getBinaryPath(platformName = platform, archName = arch) {
    const pkgName = `maglev-${platformName}-${archName}`;

    try {
        // Try to find the platform-specific package
        const pkgPath = require.resolve(`${pkgName}/package.json`);
        const binName = platformName === 'win32' ? 'maglev.exe' : 'maglev';
...
function reportUnsupportedPlatform(platformName = platform, archName = arch, log = console.error) {
    ...
    if (RELEASE_URL) {
        ...
        log('No public Maglev binary download URL is configured for this build.');
    }
}

function reportMissingPlatformPackage(platformName = platform, archName = arch, log = console.error) {
    const platformPackage = `maglev-${platformName}-${archName}`;
    ...
    log(`  npm install -g maglev --registry=${OFFICIAL_NPM_REGISTRY}`);
    log('');
    if (RELEASE_URL) {
