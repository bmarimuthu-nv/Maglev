...
async function runnerPost(path: string, body?: any): Promise<{ error?: string } | any> {
  ...
  try {
    const timeout = process.env.MAGLEV_RUNNER_HTTP_TIMEOUT ? parseInt(process.env.MAGLEV_RUNNER_HTTP_TIMEOUT) : 10_000;
 ...
 * For instance we are not handling the case where we upgraded maglev,
 ...
 * For instance when running `maglev runner status` we can show more information.
 */
 ...
 */
export async function isRunnerRunningCurrentCliVersion(): Promise<boolean> {
  ...
  try {
    ...
    // we will get a new path or not when maglev is upgraded globally.
    ...
    /*
    const { spawnMaglevCli } = await import('@/utils/spawnMaglevCli');
    const maglevProcess = spawnMaglevCli(['--version'], { stdio: 'pipe' });
    let version: string | null = null;
    maglevProcess.stdout?.on('data', (data) => {
      version = data.toString().trim();
    });
    await new Promise(resolve => maglevProcess.stdout?.on('close', resolve));
