 ...
 * - MAGLEV_HOME=~/.maglev-dev-test (DIFFERENT from dev's ~/.maglev-dev!)
 * - MAGLEV_API_URL=http://localhost:3006 (local maglev-hub)
 * - MAGLEV_API_TOKEN=... (must match the hub)
 */
...
async function isServerHealthy(): Promise<boolean> {
  try {
    if (!configuration.cliApiToken) {
      console.log('[TEST] Missing MAGLEV_API_TOKEN (required for direct-connect integration tests)');
      return false;
    }
    ...
    if (response.status === 401) {
      console.log('[TEST] Bot health check failed: invalid MAGLEV_API_TOKEN');
      return false;
    }
...
describe.skipIf(!await isServerHealthy())('Runner Integration Tests', { timeout: 20_000 }, () => {
  ...
  beforeEach(async () => {
    ...
    void spawnMaglevCli(['runner', 'start'], {
      stdio: 'ignore'
    });
  ...
  it('should track session-started webhook from terminal session', async () => {
    ...
    const mockMetadata: Metadata = {
      ...
      host: 'test-host',
      homeDir: '/test/home',
      maglevHomeDir: '/test/maglev-home',
      maglevLibDir: '/test/maglev-lib',
      maglevToolsDir: '/test/maglev-tools',
      hostPid: 99999,
      startedBy: 'terminal',
    ...
    expect(tracked.startedBy).toBe('maglev directly - likely by user from terminal');
    expect(tracked.sessionId).toBe('test-session-123');
  ...
  it('should spawn & stop a session via HTTP (not testing RPC route, but similar enough)', async () => {
    ...
    const spawnedSession = sessions.find(
      (s: any) => s.sessionId === response.sessionId
    );
    ...
    await stopRunnerSession(spawnedSession.sessionId);
  });
  ...
  it('should track both runner-spawned and terminal sessions', async () => {
    // Spawn a real maglev process that looks like it was started from terminal
    const terminalMaglevProcess = spawnMaglevCli([
      '--maglev-starting-mode', 'remote',
      '--started-by', 'terminal'
    ], {
      cwd: '/tmp',
      detached: true,
    ...
    if (!terminalMaglevProcess || !terminalMaglevProcess.pid) {
      throw new Error('Failed to spawn terminal maglev process');
    }
    ...
    const terminalSession = sessions.find(
      (s: any) => s.pid === terminalMaglevProcess.pid
    );
    const runnerSession = sessions.find(
      (s: any) => s.sessionId === spawnResponse.sessionId
    );

    expect(terminalSession).toBeDefined();
    expect(terminalSession.startedBy).toBe('maglev directly - likely by user from terminal');
    ...
    await stopRunnerSession(runnerSession.sessionId);
    
    // Also kill the terminal process directly to be sure
    try {
      await killProcessByChildProcess(terminalMaglevProcess);
    } catch (e) {
  ...
  it('should update session metadata when webhook is called', async () => {
    ...
    const sessions = await listRunnerSessions();
    const session = sessions.find((s: any) => s.sessionId === spawnResponse.sessionId);
  ...
  it('should handle concurrent session operations', async () => {
    ...
    const runnerSessions = sessions.filter(
      (s: any) => s.startedBy === 'runner' && spawnedSessionIds.includes(s.sessionId)
    );
    ...
    for (const session of runnerSessions) {
      ...
      await stopRunnerSession(session.sessionId);
    }
  });
   ...
   * 6. Runner spawns new runner via spawnMaglevCli(['runner', 'start'])
   ...
   * 
   * This simulates what happens during `npm upgrade maglev`:
   * - Running runner has OLD version loaded in memory (configuration.currentCliVersion)
   * - npm replaces node_modules/maglev/ with NEW version files
  ...
  it('[takes 1 minute to run] should detect version mismatch and kill old runner', { timeout: 100_000 }, async () => {
    ...
    try {
      ...
      // otherwise runner will spawn a non-existing Maglev entrypoint.
      ...
      await new Promise(resolve => setTimeout(resolve, parseInt(process.env.MAGLEV_RUNNER_HEARTBEAT_INTERVAL || '30000') + 10_000));
  ...
  // TODO: Test npm uninstall scenario - runner should gracefully handle when maglev is uninstalled
