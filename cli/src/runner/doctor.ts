...
/**
 * Find all Maglev CLI processes (including current process)
 */
export async function findAllMaglevProcesses(): Promise<Array<{ pid: number, command: string, type: string }>> {
  try {
    ...
    for (const proc of processes) {
      ...
      // Check if it's a Maglev process
      const isMaglevBinary = /(?:^|\/)(?:nv-)?maglev(?:\.exe)?$/.test(name) || /\b(?:nv-)?maglev(?:\.exe)?\b/.test(cmd);
      // Dev mode: running via bun/node with src/index.ts (production uses compiled binary)
      const isDevMode = cmd.includes('src/index.ts');
      const isMaglevProcess = isMaglevBinary || isDevMode;
      
      if (!isMaglevProcess) continue;
...
/**
 * Find all runaway Maglev CLI processes that should be killed
 */
export async function findRunawayMaglevProcesses(): Promise<Array<{ pid: number, command: string }>> {
...
/**
 * Kill all runaway Maglev CLI processes
 */
export async function killRunawayMaglevProcesses(): Promise<{ killed: number, errors: Array<{ pid: number, error: string }> }> {
  const runawayProcesses = await findRunawayMaglevProcesses();
