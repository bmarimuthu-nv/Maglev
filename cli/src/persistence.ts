/**
 * Minimal persistence functions for Maglev CLI
 * 
 * Handles settings, encryption key, and runner state storage in ~/.maglev/ (or MAGLEV_HOME override)
 */
...
interface Settings {
  ...
  runnerAutoStartWhenRunningMaglev?: boolean
  cliApiToken?: string
  // API URL for server connections (priority: env MAGLEV_API_URL > this > default)
  apiUrl?: string
...
export async function writeSettings(settings: Settings): Promise<void> {
  if (!existsSync(configuration.maglevHomeDir)) {
    await mkdir(configuration.maglevHomeDir, { recursive: true })
  }
