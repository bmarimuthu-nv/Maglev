...
export const runnerCommand: CommandDefinition = {
    ...
    run: async ({ commandArgs }) => {
        ...
        if (runnerSubcommand === 'start') {
            const child = spawnMaglevCli(['runner', 'start-sync'], {
                detached: true,
                stdio: 'ignore',
                env: process.env
            })
        ...
        console.log(`
${chalk.bold('maglev runner')} - Runner management

${chalk.bold('Usage:')}
  maglev runner start              Start the runner (detached)
  maglev runner stop               Stop the runner (sessions stay alive)
  maglev runner status             Show runner status
  maglev runner list               List active sessions

  If you want to kill all maglev related processes run 
  ${chalk.cyan('maglev doctor clean')}

${chalk.bold('Note:')} The runner runs in the background and manages Claude sessions.

${chalk.bold('To clean up runaway processes:')} Use ${chalk.cyan('maglev doctor clean')}
`)
    }
}
