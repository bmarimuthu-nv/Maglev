 ...
 *
 * Automatically starts the Maglev hub when CLI is launched
 * if specific conditions are met:
 * 1. MAGLEV_API_URL is not set (using default localhost:3006)
...
async function shouldAutoStartServer(): Promise<boolean> {
    // Condition 1: MAGLEV_API_URL not set (using default localhost:3006)
    if (process.env.MAGLEV_API_URL) {
        logger.debug('[AUTO-START] MAGLEV_API_URL is set, skipping auto-start')
        return false
    }
...
export async function maybeAutoStartServer(): Promise<void> {
    try {
        ...
        console.log(chalk.gray('Starting Maglev hub in background...'))
        ...
        if (!isReady) {
            ...
            console.log(chalk.gray('  Try running `maglev hub` manually to see errors'))
            return
        }

        console.log(chalk.green('Maglev hub started'))
    } catch (error) {
