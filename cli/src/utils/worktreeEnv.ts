...
function readWorktreeFromEnv(): WorktreeInfo | null {
    const basePath = process.env.MAGLEV_WORKTREE_BASE_PATH?.trim();
    const branch = process.env.MAGLEV_WORKTREE_BRANCH?.trim();
    const name = process.env.MAGLEV_WORKTREE_NAME?.trim();
    const worktreePath = process.env.MAGLEV_WORKTREE_PATH?.trim();
    const createdAtRaw = process.env.MAGLEV_WORKTREE_CREATED_AT?.trim();
