/**
 * Cross-platform Maglev CLI spawning utility
 *
 * ## Background
 *
 * Maglev CLI runs in two modes:
 ...
 *
 * This utility handles spawning Maglev CLI subprocesses (for runner processes)
...
export interface MaglevCliCommand {
  command: string;
  args: string[];
}

export function getMaglevCliCommand(args: string[]): MaglevCliCommand {
...
export function spawnMaglevCli(args: string[], options: SpawnOptions = {}): ChildProcess {
  ...
  // bypassing the 'maglev' wrapper that would normally be found in the shell's PATH.
  // However, we log it as 'maglev' here because other engineers are typically looking
  // for when "maglev" was started and don't care about the underlying node process
  // details and flags we use to achieve the same result.
  const fullCommand = `maglev ${args.join(' ')}`;
  logger.debug(`[SPAWN Maglev CLI] Spawning: ${fullCommand} in ${directory}`);
  
  const { command: spawnCommand, args: spawnArgs } = getMaglevCliCommand(args);
  ...
  if (!isBunCompiled()) {
    ...
    if (entrypoint && !existsSync(entrypoint)) {
      ...
      logger.debug(`[SPAWN Maglev CLI] ${errorMessage}`);
