...
function extractProcessLogPid(filename: string): number | null {
  const match = filename.match(/-pid-(\d+)(?:-runner)?\.log$/)
  if (!match) {
    return null
  }
...
function isEphemeralProcessLog(filename: string): boolean {
  return !filename.startsWith('hub-') && /-pid-\d+(?:-runner)?\.log$/.test(filename)
}

function pruneUnusedProcessLogs(): void {
  try {
    if (!existsSync(configuration.logsDir)) {
      return
    }

    for (const file of readdirSync(configuration.logsDir)) {
      if (!isEphemeralProcessLog(file)) {
        continue
      }

      const pid = extractProcessLogPid(file)
      if (!pid || isProcessAlive(pid)) {
        continue
      }

      try {
        rmSync(join(configuration.logsDir, file), { force: true })
      } catch {
        // Best-effort cleanup only.
      }
    }
  } catch {
    // Ignore cleanup failures to avoid affecting session startup.
  }
}
...
class Logger {
  ...
  constructor(
    ...
    pruneUnusedProcessLogs()
    // Remote logging enabled only when explicitly set with API URL
    if (process.env.DANGEROUSLY_LOG_TO_SERVER_FOR_AI_AUTO_DEBUGGING
      && process.env.MAGLEV_API_URL) {
      this.dangerouslyUnencryptedServerLoggingUrl = process.env.MAGLEV_API_URL
  ...
  debug(message: string, ...args: unknown[]): void {
    ...
    // NOTE: console debug output can break interactive shell usage.
