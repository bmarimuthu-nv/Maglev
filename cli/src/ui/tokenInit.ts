 ...
 *
 * Handles MAGLEV_API_TOKEN initialization with priority:
 * 1. Environment variable (highest - allows temporary override)
 * 2. Settings file (~/.maglev/settings.json)
...
export async function initializeToken(): Promise<void> {
    ...
    if (!process.stdin.isTTY) {
        throw new Error('MAGLEV_API_TOKEN is required. Set it via environment variable or run `maglev auth login`.')
    }
