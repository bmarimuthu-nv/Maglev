...
export function buildMachineMetadata(): MachineMetadata {
    return {
        host: process.env.MAGLEV_HOSTNAME || os.hostname(),
        platform: os.platform(),
        maglevCliVersion: packageJson.version,
        homeDir: os.homedir(),
        maglevHomeDir: configuration.maglevHomeDir,
        maglevLibDir: runtimePath()
    }
}
...
export function buildSessionMetadata(options: {
    ...
    return {
        ...
        maglevHomeDir: configuration.maglevHomeDir,
        maglevLibDir,
        maglevToolsDir: resolve(maglevLibDir, 'tools', 'unpacked'),
...
async function getMachineIdOrExit(): Promise<string> {
    const envMachineId = process.env.MAGLEV_MACHINE_ID?.trim()
    if (envMachineId) {
        logger.debug(`Using machineId from MAGLEV_MACHINE_ID: ${envMachineId}`)
        return envMachineId
    }
    ...
    if (!machineId) {
        const repositoryUrl =
            typeof packageJson.repository === 'object' && packageJson.repository?.url
                ...
                : 'the Maglev repository'
        console.error(`[START] No machine ID found in settings, which is unexpected since authAndSetupMachineIfNeeded should have created it. Please report this issue on ${repositoryUrl}`)
        process.exit(1)
    }
