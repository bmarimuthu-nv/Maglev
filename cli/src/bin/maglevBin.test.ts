...
const binModulePath = path.resolve(process.cwd(), 'bin/maglev.cjs');
const {
    formatCommand,
    isSupportedPlatform,
    normalizeExecError,
...
describe('maglev binary launcher error reporting', () => {
    it('formats command with shell-safe JSON quoting', () => {
        const command = formatCommand('/tmp/maglev', ['serve', '--name', 'my agent']);
        expect(command).toBe('"/tmp/maglev" "serve" "--name" "my agent"');
    });
    ...
    it('normalizes child process execution errors', () => {
        const normalized = normalizeExecError({
            status: 132,
            signal: 'SIGILL',
            message: 'Command failed: /tmp/maglev',
        });
    ...
    it('reports execution details before exit handling', () => {
        ...
        expect(lines).toEqual([
            'Failed to execute: "/tmp/maglev" "serve" "--port" "3000"',
    ...
    it('handles unknown failures with generic output', () => {
        ...
        const result = reportExecutionFailure({}, '/tmp/maglev', [], (line: string) => {
            lines.push(line);
        });

        expect(result).toEqual({ status: null, signal: null });
        expect(lines).toEqual(['Failed to execute: "/tmp/maglev"']);
    });
    ...
    it('reports missing platform package with installation guidance', () => {
        ...
        expect(lines).toContain('Missing platform package: maglev-linux-x64');
        expect(lines).toContain('Try reinstalling with the official npm registry:');
        expect(lines).toContain('  npm install -g maglev --registry=https://registry.npmjs.org');
        expect(lines).toContain('No public Maglev binary download URL is configured for this build.');
    });
});
