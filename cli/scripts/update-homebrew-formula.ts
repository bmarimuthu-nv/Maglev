/**
 * Update the Homebrew formula for maglev.
 *
 * This script:
 * 1. Reads checksums from release-artifacts/checksums.txt
 * 2. Generates an updated maglev.rb formula
 ...
 *
 * Environment:
 *   MAGLEV_REPOSITORY_URL - Browser URL for the Maglev repository (required)
 *   MAGLEV_RELEASE_BASE_URL - Base release download URL (required)
...
class Maglev < Formula
  ...
  on_macos do
    if Hardware::CPU.arm?
      url "${releaseBaseUrl}/v#{version}/maglev-darwin-arm64.tar.gz"
      sha256 "${shas.darwinArm64}"
    else
      url "${releaseBaseUrl}/v#{version}/maglev-darwin-x64.tar.gz"
  ...
  on_linux do
    if Hardware::CPU.arm?
      url "${releaseBaseUrl}/v#{version}/maglev-linux-arm64.tar.gz"
      sha256 "${shas.linuxArm64}"
    else
      url "${releaseBaseUrl}/v#{version}/maglev-linux-x64.tar.gz"
  ...
  test do
    assert_match version.to_s, shell_output("#{bin}/maglev --version")
  end
end
`;
}
...
async function main(): Promise<void> {
    ...
    const repositoryUrl = requireEnv('MAGLEV_REPOSITORY_URL');
    const releaseBaseUrl = requireEnv('MAGLEV_RELEASE_BASE_URL');
    const tapRepo = process.env.HOMEBREW_TAP_REPO?.trim();
    ...
    const formulaContent = generateFormula(version, shas, repositoryUrl, releaseBaseUrl);
    ...
    if (!shouldPush) {
        ...
        const localFormulaPath = join(localFormulaDir, 'maglev.rb');
    ...
    if (!tapRepo) {
        throw new Error('Missing required environment variable: HOMEBREW_TAP_REPO');
    }
    ...
    try {
        ...
        // Write formula
        const formulaPath = join(formulaDir, 'maglev.rb');
        ...
        // Commit and push
        execSync('git add Formula/maglev.rb', { cwd: tempDir, stdio: 'pipe' });

        try {
            execSync(`git commit -m "Update maglev to v${version}"`, { cwd: tempDir, stdio: 'pipe' });
            execSync('git push origin main', { cwd: tempDir, stdio: 'pipe' });
            console.log(`\nSuccessfully pushed maglev v${version} to homebrew-tap`);
        } catch {
            console.log('\nNo changes to commit (formula already up to date)');
        }

        console.log('\nUsers can now install via:');
        console.log('  brew install tiann/tap/maglev');
    } finally {
        // Cleanup
